...
extern "C" int decrypt_enclave(int decrypt)
{
   ...
   for (uint32_t i = 0; i < num_rvas; i++) {
	...
	ret = sgx_rijndael128GCM_decrypt((const sgx_aes_gcm_128bit_key_t *)key, (const uint8_t *)ciphertext,
                                        size, plaintext, iv, 12,
                                        NULL, 0, (const sgx_aes_gcm_128bit_tag_t *)tag);
